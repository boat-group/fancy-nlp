# -*- coding: utf-8 -*-
# crf layer that works with tf2.x，originally forked from
# https://github.com/howl-anderson/addons/blob/feature/crf_layers/tensorflow_addons/layers/crf.py
...
class CRF(tf.keras.layers.Layer):
    ...
    ```python
        ...
        from metrics import crf_accuracy
        model = Sequential()
        model.add(Embedding(3001, 300, mask_zero=True)
        crf = CRF(10)
        model.add(crf)
        model.compile('adam', loss=crf_loss, metrcis=[crf_accuracy])
        model.fit(x, y)
    ```
    Arguments:
    ...
    Output shape:
        3D tensor (dtype: int32) with shape: `(batch_size, sequence_length, tag_num)`.
    Masking:
    ...
    def get_negative_log_likelihood(self, y_true):
        # covert 3D ont-hot encoded tensor to 2D tensor, which represents the matrxi
        # of tag indices for which we compute the log-likehood
    ...
    def get_accuracy(self, y_true, y_pred):
        # covert 3D ont-hot encoded tensor to 2D tensor
        y_true = tf.keras.backend.argmax(y_true, axis=-1)
